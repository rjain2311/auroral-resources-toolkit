...
public class DMSPHeader
{
          ...
          throws IOException
  {
    ...
    //}
    if ( numHeaderRecords + numDataRecordsDim.getLength() != numRecords )
    {
      throw new IOException( "Invalid DMSP file: the number of header records <" + this.numHeaderRecords + "> and data records <" + this.numDataRecordsDim.getLength() + "> is not equal to total records <" + this.numRecords + ">." );
    }
  ...
  private void handleProcessingInformation()
          throws IOException
  {
		//jjn suborbitHistory can be missing
    String suborbitHistoryAttString = (String) headerInfo.get( HeaderInfoTitle.SUBORBIT_HISTORY.toString() );
    if ( suborbitHistoryAttString != null )
		  suborbitHistoryAtt = new Attribute( this.suborbitHistoryAttName, suborbitHistoryAttString );
  ...
  private void handleOrbitInformation() throws IOException
  {
    ...
    //jjn F14 has no ACCENDING_NODE attribute in header
    String ascendingNodeString = (String) headerInfo.get( HeaderInfoTitle.ASCENDING_NODE.toString());
    Double ascendingNode = new Double(Double.NaN);
    if (ascendingNodeString != null) {
	    ascendingNode = Double.valueOf( ascendingNodeString );
    }
